...
export const fetchReviewsByLatest = async (onlyTrueHashtagObj, idsOfCafesInMap, viewAll) => {
  if (viewAll) {
    if (Object.keys(onlyTrueHashtagObj).length === 0) {
      const response = await supabase.from('reviews').select().eq('해시태그 없음', true);
    ...
    if (Object.keys(onlyTrueHashtagObj).length === 0) {
      const response = await supabase.from('reviews').select().eq('해시태그 없음', true).in('cafe_id', idsOfCafesInMap);
      // .order('created_at', { ascending: false });
      return response.data;
    } else if (Object.keys(onlyTrueHashtagObj).length === 6) {
      const response = await supabase
